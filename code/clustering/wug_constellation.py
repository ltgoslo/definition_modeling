...
from wug_cluster import get_clusters
...
class Constellation(object):
    ...
    def __init__(self, distribution1=None, distribution2=None, old='old', new='new', graph=None, is_cluster=False,
                 algorithm=None, threshold=0.5, bound1=1, bound2=1, lowerbound1=1, lowerbound2=1, is_prob=False):
        ...
        if self.distribution1 != None and self.distribution2 != None:
            ...
            self._make_change_scores(bound1=bound1, bound2=bound2, lowerbound1=lowerbound1, lowerbound2=lowerbound2,
                                     is_prob=is_prob)
    ...
    def _make_graph_stats(self, old='old', new='new', attributes={'type': 'usage'}):
        ...
        self.nodes = [node for node in self.G.nodes if
                      all([self.G.nodes()[node][k] == v for (k, v) in attributes.items()])]
        ...
        self.restnodes = [node for node in self.nodes if (not self.G.nodes()[node]['grouping'] == new) and (
            not self.G.nodes()[node]['grouping'] == old)]
    ...
    def _make_dist_stats(self, bound1=1, bound2=1, lowerbound1=1, lowerbound2=1, is_prob=False):
        ...
        self.newuseno = np.sum(
            [f2 for (f1, f2) in zip(distribution1, distribution2) if f1 <= lowerbound1 and f2 >= bound2])
        self.olduseno = np.sum(
            [f1 for (f1, f2) in zip(distribution1, distribution2) if f2 <= lowerbound2 and f1 >= bound1])
        self.newmeaningno = int(
            np.sum([1 for (f1, f2) in zip(distribution1, distribution2) if f1 <= lowerbound1 and f2 >= bound2]))
        self.oldmeaningno = int(
            np.sum([1 for (f1, f2) in zip(distribution1, distribution2) if f2 <= lowerbound2 and f1 >= bound1]))
    ...
    def _make_change_scores(self, bound1=1, bound2=1, lowerbound1=1, lowerbound2=1, is_prob=False):
        ...
        self._make_dist_stats(bound1=bound1, bound2=bound2, lowerbound1=lowerbound1, lowerbound2=lowerbound2,
                              is_prob=is_prob)
    ...
    def _make_graph(self, old='old', new='new'):
        ...
        for (n1, n2) in product(range(clustersno), repeat=2):
            ...
            # Weight between clusters
                ...
                for (i, j) in product(clustersold[cname1] + clustersnew[cname1],
                                      clustersold[cname2] + clustersnew[cname2]):
