...
def transform_judgments(G, non_value=0.0, transformation=lambda x: x):
    ...
    for (i, j) in G.edges():
        ...
        data = {a: [transformation(v) if not v == non_value else float('nan') for v in vs] for (a, vs) in
                judgments.items()}
...
def make_meta_graph(graph, test_statistic=np.nanmedian):
    ...
    node_attribute_keys = list(
        graph.nodes()[list(graph.nodes().keys())[0]].keys())  # infer node format from first node in input graph
