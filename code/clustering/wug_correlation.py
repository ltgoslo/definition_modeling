...
def cluster_correlation_search(G, s=10, max_attempts=200, max_iters=5000, initial=[], split_flag=True):
    ...
    if loss_init == 0.0:
        ...
        end_time = time.time()
        stats = stats | {'s': s, 'max_attempts': max_attempts, 'max_iters': max_iters, 'split_flag': split_flag,
    ...
    # `pool.apply`
    solutions = pool.starmap(Linear_loss.optimize_simulated_annealing,
                             [(n, classes, G.nodes(), init_state, max_attempts, max_iters) for n in range(2, s)])
    pool.close()
    # print(solutions[0])
...
class Loss(object):
    ...
    def __init__(self, fitness_fn, edges_positive=None, edges_negative=None, edges_min=None, edges_max=None,
                 signs=None):
    ...
    def optimize_simulated_annealing(self, n, classes, nodes, init_state, max_attempts, max_iters):
        ...
        best_state, best_fitness = mlrose.simulated_annealing(problem, schedule=schedule, init_state=init_state,
        ...
        schedule = mlrose.ExpDecay()
        best_state, best_fitness = mlrose.simulated_annealing(problem, schedule=schedule, max_attempts=max_attempts,
                                                              max_iters=max_iters)
