...
import random
import numpy as np
from wug_constellation import Constellation
from wug_cluster import get_clusters, get_uncompared_clusters, get_low_prob_clusters, transform_edge_weights, \
    transform_judgments
from wug_correlation import Loss
from scipy.stats import spearmanr, pearsonr
from scipy.spatial.distance import euclidean, cosine
import wug_krippendorff as krippendorff
...
def make_weights(G, annotators, summary_statistic=np.median, non_value=0.0, normalization=lambda x: x):
    ...
    for (i, j) in G.edges():
        ...
        values = [summary_statistic(judgments[annotator]) for annotator in annotators if
                  annotator in judgments]  # take median of multiple judgments of same annotator
...
def get_edge_std(G, annotators, non_value=0.0, normalization=lambda x: ((x - 1) / 3.0)):
    ...
    combo2judgments_clean = {c: [normalization(j) for j in js if j != non_value and not np.isnan(j)] for (c, js) in
                             combo2judgments.items()}
...
def get_empty_edges(G, annotators):
    ...
    empty_edges = [(i, j) for (i, j) in G.edges() if
                   not any([True for annotator in annotators if annotator in G[i][j]['judgments']])]
...
def get_annotator_conflicts(G, annotators, non_value=0.0, threshold=0.5, normalization=lambda x: x,
...
def get_annotator_overlap(combo2annotator2judgment, annotators, summary_statistic=np.median):
    ...
    for (c, annotator2judgment) in combo2annotator2judgment.items():
        data = [(a, d) for a in annotators for d in [summary_statistic(annotator2judgment[a])] if
                not np.isnan(d)]  # judgment overlap
...
def get_excluded_nodes(node2judgments, node2weights, non_value=0.0, share=1.0, is_non_value=lambda x: np.isnan(x)):
    ...
    nodes_zero = [node for node in node2judgments if
                  len([judgment for judgment in node2judgments[node] if judgment == non_value]) / len(
                      node2judgments[node]) >= share]  # nodes with at least half zero-judgments
    nodes_nan = [node for node in node2weights if
                 len([weight for weight in node2weights[node] if is_non_value(weight)]) == len(
                     node2weights[node])]  # nodes with only nan edges
...
def get_data_maps_edges(G, annotators, summary_statistic=np.median):
    ...
    for (i, j) in G.edges():
        judgments = G[i][j]['judgments']
        comments = G[i][j]['comments']
        judgments = {annotator: summary_statistic(judgments[annotator]) for annotator in annotators if
                     annotator in judgments}
    ...
    annotator2judgments = {annotator: [
        summary_statistic(combo2annotator2judgment[(i, j)][annotator]) if annotator in combo2annotator2judgment[
            (i, j)] else float('nan') for (i, j) in combo2annotator2judgment.keys()] for annotator in annotators}

    combo2judgments = {
        combo: [combo2annotator2judgment[combo][annotator] for annotator in combo2annotator2judgment[combo]] for combo
        in combo2annotator2judgment.keys()}
...
def get_graph_stats(G, annotators, non_value=0.0, annotation_values=range(5), share=0.5, value_domain=None,
                    expected=None, limit=300, metrics=['kri', 'kri2', 'coh', 'spr', 'ham', 'prs', 'eud']):
    """
    ...
    mappings_edges = get_data_maps_edges(G, annotators)
    combo2annotator2judgment, annotator2judgments, node2judgments, node2weights = mappings_edges[
                                                                                      'combo2annotator2judgment'], \
                                                                                  mappings_edges['annotator2judgments'], \
                                                                                  mappings_edges['node2judgments'], \
                                                                                  mappings_edges['node2weights']
...
def get_agreements(annotator2judgments, non_value=0.0, level_of_measurement='ordinal', value_domain=None, expected=None,
                   metrics=['kri', 'kri2', 'coh', 'spr', 'ham', 'prs', 'eud'], combo2annotator2judgment=None,
                   is_data=False):
    """
    ...
    for (anno1, anno2) in combinations(annotator2judgments.keys(), 2):
        ...
        if 'kri' in metrics:
            ...
            try:
                # print(list(data1))
                # print(list(data2))
                kri = krippendorff.alpha(reliability_data=reliability_data, level_of_measurement=level_of_measurement,
        ...
        if 'kri2' in metrics:
            ...
            try:
                kri = krippendorff.alpha(reliability_data=reliability_data, level_of_measurement=level_of_measurement,
                                         value_domain=value_domain, expected=expected)
    ...
    for metric in metrics:
        ...
        overlap_total_non_nan = np.sum(
            [overlap for pair, overlap in stats['overlap'].items() if not np.isnan(stats[metric][pair])])
        mean_weighted = np.nansum(
            [(stats['overlap'][annotators] / overlap_total_non_nan) * stats[metric][annotators] for annotators in
             stats['overlap']])  # todo: take care of cases where overlap is 1
    ...
    for i in column2others:
        ...
        if 'kri' in metrics:
            reliability_data = [data1, data2]
            try:
                kri = krippendorff.alpha(reliability_data=reliability_data,
                                         level_of_measurement=level_of_measurement)  # currently not done with external expected distribution, as value_domain changes for averages
            except AssertionError as e:
                kri = float('nan')
...
def get_cluster_stats(G, threshold=0.5, min_val=0.0, max_val=1.0, is_non_value=lambda x: np.isnan(x),
                      loss_function='linear_loss', noise_label=-1):
    """
    ...
    edges_positive = set(
        [(n2i[i], n2i[j], w - threshold) for (i, j, w) in G_clean.edges.data("weight") if w >= threshold])
    edges_negative = set(
        [(n2i[i], n2i[j], w - threshold) for (i, j, w) in G_clean.edges.data("weight") if w < threshold])
    ...
    between_conflicts = Loss('binary_loss', edges_positive=edges_positive, edges_negative=edges_negative,
                             signs=['pos']).loss(cluster_state)
    within_conflicts = Loss('binary_loss', edges_positive=edges_positive, edges_negative=edges_negative,
                            signs=['neg']).loss(cluster_state)
    ...
    uncompared_high_prob_cluster_combs = [(c1, c2) for (c1, c2) in uncompared_cluster_combs if
                                          not (c1 in low_prob_clusters or c2 in low_prob_clusters)]
...
def get_time_stats(G, threshold=0.5, lower_range=(1, 3), upper_range=(3, 5), lower_prob=0.01, upper_prob=0.1, old='old',
                   new='new', attributes={'type': 'usage'}):
    """
    ...
    try:
        co = Constellation(graph=G, bound1=upperbound1, bound2=upperbound2, lowerbound1=lowerbound1,
                           lowerbound2=lowerbound2, is_prob=False, old=old, new=new, threshold=threshold)
        is_clusters = True
    except KeyError:
        print('No clusters found.')
        is_clusters = False
    ...
    wearlier = [d['weight'] for (u, v, d) in G_target.edges(data=True) if
                G_target.nodes()[u]['grouping'] == old and G_target.nodes()[v]['grouping'] == old]
    wlater = [d['weight'] for (u, v, d) in G_target.edges(data=True) if
              G_target.nodes()[u]['grouping'] == new and G_target.nodes()[v]['grouping'] == new]
    wcompare = [d['weight'] for (u, v, d) in G_target.edges(data=True) if
                (G_target.nodes()[u]['grouping'] == old and G_target.nodes()[v]['grouping'] == new) or (
                            G_target.nodes()[u]['grouping'] == new and G_target.nodes()[v]['grouping'] == old)]
    # print(wearlier)
